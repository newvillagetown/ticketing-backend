...
func (s *Suite) Test_repository_FindProduct() {
	...
	rows := sqlmock.NewRows([]string{"name", "description", "category", "per_amount", "img_url", "total_count", "rest_count", "start_date", "end_date"}).
		AddRow(mockProducts[0].Name, mockProducts[0].Description, mockProducts[0].Category,
		...
		AddRow(mockProducts[1].Name, mockProducts[1].Description, mockProducts[1].Category,
